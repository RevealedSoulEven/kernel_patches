...
/*static inline bool is_need_bypass(void)
{
...
char *get_file_path_str(struct file *file)
{
     ...
     * so we shift the data to start at the buffer’s beginning
...
static int bypass_show_map_vma(struct vm_area_struct *vma) {
    ...
    if (strstr(file->f_path.dentry->d_iname, "frida-"))
        return 1;
    if (strstr(file->f_path.dentry->d_iname, "libart.so") && (flags & VM_EXEC)) {
   /* char buf[256]; 
    snprintf(buf, sizeof(buf), "MAP_PATCH %s  -  %s", file->f_path.dentry->d_iname, );
    pr_info("%s", buf);*/
        return 1; } 
  /*  if (file && file->f_path.dentry && (strstr(file->f_path.dentry->d_iname, "memfd:jit-cache") || strstr(file->f_path.dentry->d_iname, "memfd:jit-zygote-cache")))
   ...
   /* char buf[256]; 
    snprintf(buf, sizeof(buf), "MAP_PATCH %s", file->f_path.dentry->d_iname);
    pr_info("%s", buf);*/
        return 1;
        }
    ...
    if (strstr(fullname, "libart.so") && (flags & VM_EXEC)) {
        //char buf[256]; 
       // snprintf(buf, sizeof(buf), "MAP_PATCH %s    -X", fullname);
      //  pr_info("%s", buf);
        return 1;
    }
    // if (strstr(fullname, "/dev/zero (deleted)")) {
    if (strstr(fullname, "/dev/zero") || strstr(fullname, "lsposed")) {
    //   char buf[256]; 
    //   snprintf(buf, sizeof(buf), "MAP_PATCH %s", fullname);
    //   pr_info("%s", buf);
        return 1;
    }*/
    return 0;
...
// normal
	...
	if (!name) {
	    ...
	    // if ((flags & VM_EXEC) && is_need_bypass()) {
	        //char buf[256];
	       // snprintf(buf, sizeof(buf), "MAP_PATCH [vdso]");
            //pr_info("%s", buf);
            name = "[vdso]";
            goto done;
        }
